...
export function LeggoForm(
  props: React.PropsWithoutRef<{ leggo: Leggo } & FormProps>
) {
  const { leggo, onFieldsChange, ...overlapFormProps } = props;
  const { formProps, schemaList } =
    leggoStores.get(leggo.ref)?.schemaModel || {};

  const handleFieldsChange = (changedFields: any[], allFields: any[]) => {
    changedFields.forEach(({ name, value }) => {
      const changedSchema = schemaList.find(
        (schema) => schema.getStringedName() === String(name)
      );
      if (changedSchema) {
        changedSchema.currentItemValue = value;
        changedSchema.linkingStringedNames.forEach((linkingName) => {
          const targetSchema = schemaList.find(
            (schema) => schema.getStringedName() === linkingName
          );
