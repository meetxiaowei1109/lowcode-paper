...
export default function LeggoItem(
  ...
  const { itemProps, inputProps, extra, postman, Successor, SuperSuccessor } =
    configs;
  const postmanParamsValueList =
    postman?.params?.map((item) => item.value) || [];
  const postmanDataValueList = postman?.data?.map((item) => item.value) || [];
  const StandardInput =
    leggoItemStore.total[type]?.StandardInput || (() => <div />);
  ...
  useMemo(() => {
    ...
    Object.values(needDefineGetterProps).forEach((getterInfo) => {
      ...
      const linkedSchema = schemaList.find(
        (schema) => schema.getStringedName() === observedStringedName
      );
      //@ts-ignore
      const targetProp = namepath
        .slice(0, -1)
        .reduce((pre, cur) => pre[cur], configs);
      const targetKey = namepath.slice(-1)[0];
      const targetType = typeof targetProp[targetKey];
      !isFromPublicStates &&
        linkedSchema.linkingStringedNames.add(schema.getStringedName());
      Reflect.defineProperty(targetProp, targetKey, {
        ...
        get: () => {
          ...
          if (isFromPublicStates) {
            ...
            return typeof publicState === "function"
              ? publicState()
              : publicState;
          }
          if (reference && rule) {
  ...
  useEffect(() => {
    ...
    if (method && url) {
      ...
      axios({ method, url, params: paramsParsed, data: dataParsed }).then(
        (res) => {
          //@ts-ignore
          const targetValue = responseNamepath
            .split(".")
            .slice(1)
            .reduce((pre, cur) => pre?.[cur], res);
